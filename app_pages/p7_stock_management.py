...
def get_inventory_data(db: Session, user_id: int):
    ...
    results = db.query(Ingredient, stock_levels_sq.c.current_stock).outerjoin(
        stock_levels_sq, Ingredient.id == stock_levels_sq.c.inventoryitem_id
    ).filter(Ingredient.user_id == user_id).order_by(Ingredient.name).all()
    inventory = []
    for ing, stock in results:
        stock_level = stock or Decimal('0.0')
        additions = db.query(StockAddition).join(PurchaseOrder).filter(StockAddition.inventoryitem_id == ing.id, PurchaseOrder.user_id == user_id).all()
...
def render_purchase_edit_form(db: Session, user: User):
    ...
    inventoryitems = db.query(Ingredient).filter(Ingredient.user_id == user.id).order_by(Ingredient.name).all()
    ing_map = {ing.name: ing.id for ing in inventoryitems}
    ...
    with st.form("edit_purchase_form"):
        ...
        line_items_data = [{"Ingredient": item.inventoryitem_ref.name, "Quantity (g)": float(item.quantity_added_grams), "Item Cost (€)": float(item.item_cost), "Supplier Lot #": item.supplier_lot_number or ""} for item in po.line_items]
                ...
                try:
                    ...
                    for _, row in edited_line_items.iterrows():
                        if ing_id := ing_map.get(row["Ingredient"]):
                            new_stock = StockAddition(purchase_order_id=po_id, inventoryitem_id=ing_id, quantity_added_grams=Decimal(str(row["Quantity (g)"])), item_cost=Decimal(str(row["Item Cost (€)"])), supplier_lot_number=row["Supplier Lot #"], quantity_remaining_grams=Decimal(str(row["Quantity (g)"])))
    ...
    if st.session_state.purchase_view_state == 'edit':
        ...
        with tab1:
            ...
            if not inventory_data:
                st.warning("No inventoryitems found. Add inventoryitems and record purchases to see your inventory.")
            else:
                ...
                if selection.selection.rows:
                    selected_inventoryitem_id = int(df.iloc[selection.selection.rows[0]]["id"])
                    selected_inventoryitem_name = df.iloc[selection.selection.rows[0]]["name_for_header"]
                    st.markdown("---"); st.subheader(f"Active Lots for: {selected_inventoryitem_name}")
                    active_lots = db.query(StockAddition).join(PurchaseOrder).options(joinedload(StockAddition.purchase_order_ref).joinedload(PurchaseOrder.supplier_ref)).filter(PurchaseOrder.user_id == user.id, StockAddition.inventoryitem_id == selected_inventoryitem_id, StockAddition.quantity_remaining_grams > 0).order_by(PurchaseOrder.order_date.asc()).all()
                    if not active_lots: st.info("No active lots for this inventoryitem.")
                    else:
        ...
        with tab2:
            ...
            if not inventoryitems:
                st.error("You must add an inventory item on the 'Manage Ingredients' page before you can record a purchase."); 
            else:
                        ...
                        with SessionLocal() as transaction_db:
                            try:
                                ...
                                if line_items_to_save.empty:
                                    st.error("Please add at least one valid inventory item line item."); transaction_db.rollback()
                                else:
                                    ...
                                    for _, row in line_items_to_save.iterrows():
                                        if ing_id := ing_map.get(row["Ingredient"]):
                                            new_stock = StockAddition(purchase_order_id=new_po.id, inventoryitem_id=ing_id, quantity_added_grams=Decimal(str(row["Quantity (g)"])), item_cost=Decimal(str(row["Item Cost (€)"])), supplier_lot_number=row["Supplier Lot #"], quantity_remaining_grams=Decimal(str(row["Quantity (g)"])))
                                            transaction_db.add(new_stock)
                                    transaction_db.commit()
