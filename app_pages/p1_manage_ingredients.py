# app_pages/p1_manage_ingredients.py
...
def render(db: Session, user: User, is_mobile: bool):
    ...
    st.write("Define your ingredients, their INCI names for compliance, and their types. Costing is now handled via the Stock Management page.")

    # --- MODIFIED: Fetch dynamic ingredient types ---
    ingredient_types_db = db.query(IngredientType).filter(IngredientType.user_id == user.id).order_by(IngredientType.name).all()
    type_options = [t.name for t in ingredient_types_db]

    if not type_options:
        st.warning("Please define at least one 'Ingredient Type' on the 'Manage Ingredient Types' page before adding ingredients.")
        return

    # --- MODIFIED: Eager load the ingredient_type_ref ---
    ingredients_db = db.query(Ingredient).options(joinedload(Ingredient.ingredient_type_ref)).filter(Ingredient.user_id == user.id).order_by(Ingredient.name).all()
    ...
    data_for_editor = [
        {
            ...
            "Type": ing.ingredient_type_ref.name if ing.ingredient_type_ref else None, 
    ...
    if st.button("Save Ingredient Changes", type="primary"):
        with SessionLocal() as transaction_db:
            try:
                ...
                type_map = {t.name: t.id for t in ingredient_types_db}
                ...
                for _, row in edited_df.iterrows():
                    ...
                    if not type_id:
                        st.error(f"Type '{ing_type_name}' is not valid. Skipping row for ingredient '{name}'.")
                        continue
                    ...
                    if ing_id:
                        ...
                        new_ingredient = Ingredient(user_id=user.id, **data_dict)
                        transaction_db.add(new_ingredient)
                
                transaction_db.commit()
                st.success("Ingredient changes saved successfully!")
            except IntegrityError:
                transaction_db.rollback()
                st.error("Save failed. Ingredient names must be unique.")
